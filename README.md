...
│   ├── 04_根因报告-写GM未回写cache.md      ★ 15/15 的根因与修法
...
## 🆕 更新：15/15 的根因已定位（2026-09-18）

在 **CANN 9.0.0 + Ascend 910B3** 上复现并定位到了此前十几个版本都卡在 13/15 的**真正根因**：

> **kernel 往 GM 写的结果值停在 aicore 的 cache 里，从未回写到 GM。**
> host 侧读到的是 `aclrtMalloc` 的零初始化内存。

| | |
| :--- | :--- |
| 完整分析 | **[docs/04_根因报告-写GM未回写cache.md](docs/04_根因报告-写GM未回写cache.md)** |
| 修复后的 kernel | **`op_kernel/kernel_cachefix.asc`**（平台提交 15/15，状态 Pass） |
| 关键一行 | `AscendC::DataCacheCleanAndInvalid<float, AscendC::CacheLine::ENTIRE_DATA_CACHE>(yGm);` |

**这个 bug 难查的地方在于：它的三个观测通道全都会给出"正常"的假象** ——

1. **kernel 内读回 `yGm[b]` 完全正确**（命中的是和写入同一个 cache）；
2. **本地小用例（B ≤ 4）全部通过**（y 只有 8~16 字节、核数少，撞不上）；
3. **平台只回一个"错误占比"百分比**（看不到错值是 0 还是算错了）。

平台上的表现是「某些测试点**恰好只错 1 个输出元素**」，极易被误判为精度问题或归约 bug。
详细排查路径、复现方法、以及给其他选手的建议，都写在报告里。

---

## 🆕 更新：性能优化收尾 —— 最终 15/15，均分 ≈15.0 / 100（2026-09-20）

在 15/15 的基础上做了 20 余轮性能优化，把「**瓶颈到底在哪**」彻底查清了：

| | |
| :--- | :--- |
| 完整记录（含全部失败方向） | **[docs/05_性能优化记录与最终成绩.md](docs/05_性能优化记录与最终成绩.md)** |
| 最终提交物（多一条向量通路） | **`op_kernel/kernel_vecpath.asc`**（平台提交 378646，15/15 Pass） |
| 相对 cachefix 的提升 | **测试点 1：14.15µs → 5.78µs（−59%）**，总分 ≈14.6 → **≈15.0** |

**三条硬结论**（都有 msprof 实测支撑）：

1. **瓶颈 100% 在 matmul 高阶 API 内部** —— `aic_scalar` 占用 68~95%，而真正的矩阵乘
   `aic_mac` 只有 0.9~23%。把**我们自己写的全部向量代码删掉，耗时只差 0.4%**。
2. **地板 ≈12.7µs 来自 `REGIST_MATMUL_OBJ`/初始化**，与 shape 无关；
   而评测里最优解只要 1.25µs ⇒ **小测试点的差距全在这块地板上**。
3. **得分公式是对数衰减**（`100/(1+log₁.₅(t/T))`）⇒ 大 shape 再快 30% 也只值 +0.3 分，
   **只有"把 5 倍差距压到 1 倍"才值钱**。

**⚠️ 最重要的一条教训，写给后来人：**

> 本地有两个验证工具（`run_case.sh` / `sweep.py`），但它们**都是每个用例开一个独立进程**，
> 而平台是**把 15 个 shape 放在同一个进程里依次调用**。
> **⇒ 跨 launch 的状态问题（cache 残留、事件 ID、GM 复用）在本地永远测不出来。**

我们用一条 **`kBaseM=128` + 运行期 `mbRows`** 的改动踩实了这个坑：本地 15/15 + 240 组合全绿，
平台却给出 **1/15**；换一种形态再交，变成 **TLE**；同一份代码两次结果不同 ⇒ 那是**概率性卡死**。

**判据：偶发失败（时好时坏）比稳定失败危险得多 —— 它会在某次侥幸通过后伪装成好版本。
不能复现平台调用形态的前提下，任何"偶发"的改动一律整体放弃。**
